...
def DataBase(): #обновление данных взятых из бд
    global TableFed,TablePicFed,TableArt,TablePic,All_artists,All_pictures,All_discription,All_ID
...
@bot.callback_query_handler(func=lambda call: call.data[:5] == 'delet')  # удалить все о выбранной картине
...
@bot.callback_query_handler(func=lambda call: call.data[:5] == 'redan')  # изменить описание картины
def Redact_Artists(call):
    global Name_Artists_Readct
    Name_Artists_Readct = call.data.split("redan")[1]
    message = call.message
    chat_id = message.chat.id
    bot.send_message(message.chat.id,f'Имя в данный момент: {TableArt[All_artists.index(call.data.split("redan")[1])][1]}')
...
def Redact_Artists_Update1(message): # изменить описание картины после получения сообщения
...
def Redact_Artists(call):
    ...
    message = call.message
    chat_id = message.chat.id
    bot.send_message(message.chat.id,f'Описание в данный момент: {TableArt[All_artists.index(call.data.split("redao")[1])][2]}')
    mesg = bot.send_message(message.chat.id,f'Напишите на что его заменить')
